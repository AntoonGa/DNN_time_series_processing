...
def doFit(xdata, ydata, initial, normalizationRange):
    ...
    # fit
    bounds = ([fmin, 0, taumin], [fmax, 2 * 3.14, taumax])
    popt, pcov = curve_fit(MONOfunc, xdata, ydata, p0=initGuess, bounds=bounds, maxfev=2500)
...
for selector in selectors:
    ...
    xbar = ["$F_c$", "$\sin (\phi)$ ", "$\cos (\phi)$", "$T$", "$\sigma$"]

    plt.bar(xbar, true_latent[:], width=0.55, label="Target - latent param.", color=colorsbar)
    plt.bar(xbar, latent_predDNN, width=0.41, label="DNN pred.", color=dnncolor)
    plt.bar(xbar, np.append(latent_predFIT, 0), width=0.26, label="Fit pred.", color=fitcolor)
    plt.yticks([])
    plt.legend(loc="upper left", fontsize=fontsizes - 4)
