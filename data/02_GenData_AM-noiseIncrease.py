...
We generate 10 lownoise batches with increasing noise, these will be passthrough on the first 10 epochs.
...
# saveDir = r"./temp/"
saveDir = r"./Storage/00_AM_Randomized_increaseNoise_1e5Batches/"
...
for ii in range(batchNb):
    ...
    #linearly increasing noise over the batches
    noiseParams=[0,.5*(ii/(batchNb-1))] 

    saveName = "TrainData_" + str(ii) +".npz"
    ...
    test = generator_AMFMPOLY.generator_AMFMPOLY(outputShape = outputShape, randomized=randomized, display=2,       signalParams = signalParams, noiseParams = noiseParams, driftParam =drifts, seed=seed, modTypes=['AM'] )
    ...
    meta = [randomized, "Normal 0 to 1", "512x1", "Conv1D input shape", "SinCos mapping", metaTags, timeStamp]
    
    #Data is saved as float32 in this dictionnary.
    DictToSave  = {"inputs"  : np.float32(real),
