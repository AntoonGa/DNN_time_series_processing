...
import os
path = r"D:\Dropbox\EPFL\Machine Learning\2021_01_paperRework\zTools"
os.chdir(path)
...
def unityPrepData( dataFolder, trainDataFile, latentDimension):
    ...
    if latentDimension == 3:
        targets_latent = np.delete(targets_latent, [3,4,5,6], axis=1) #No mod,tau,noise regressor
...
dataFolders = [r"D:\Dropbox\EPFL\Machine Learning\2021_01_paperRework\data\Storage\00_AM_Randomized_1e5Batches/"]

modelsFolders = [r"D:\Dropbox\EPFL\Machine Learning\2021_01_paperRework\trainModels\1610562995_TrainedModels_Latent7/"]
                             
modelNames = ["Unity_Latent7_1610562995-train_Beta0.001_AM_10x20x1_8bs-.keras",
              "Unity_Latent7_1610562995-train_Beta0.001_AM_noiseIncrease_10x20x1_8bs-.keras"]                

# mode = 'AM_increaseNoise'
mode = 'AM'
if mode == 'AM':
    modelLocation = modelsFolders[0] + modelNames[0]
    dataLocation  = dataFolders[0]
elif mode == 'AM_increaseNoise':
    modelLocation = modelsFolders[0] + modelNames[1]
    dataLocation  = dataFolders[0]
    
###########Load models
unity = keras.models.load_model( modelLocation, custom_objects={ 'customLoss': customLoss, 'mseSignal': mseSignal, 'mseLatent': mseLatent } )   
opt = unity.optimizer
...
for dataTarget in tqdm(range(nbSamples)):
        ...
        latent_true = targets[dataTarget,512:]           
        
        pred = unityOutputs[dataTarget,0:512]
        latent_pred = unityOutputs[dataTarget,512:]
...
axs[0].scatter(sortedNoise, sortedMSEunitySignal, marker="*" ,label= "DNN" ,c='orange', alpha = 0.8)
axs[0].set_yscale('log')
axs[0].legend()
axs[0].set_ylabel("$MSE_{dec}$ [-]", fontsize = fontsizes)
 
axs[1].scatter(sortedNoise, sortedMSEunityLatent,  marker="*" ,label= "DNN" ,c='orange', alpha = 0.8)
...
def doFit(xdata,ydata, initial,normalizationRange):
    ...
    def AMfunc(x,freq,phi,Fmod,Imod,Tau):
        ...
        ys = 1*cos( 2*3.14*x*freq + phi )*exp(-x/(Tau*511)) 
        ms = (1 + Imod*cos( 2*3.14*x*Fmod ))
    ...
    sinphiRescaled = ( sin(popt[1])+1)/2
    cosphiRescaled = ( cos(popt[1])+1)/2
    fmodRescaled   = ( popt[2] - fmmin  )  / (fmmax-fmmin)
    ImodRescaled   = ( popt[3] - Imin   )  / (Imax-Imin)
    tauRescaled    = ( popt[4] - taumin )  / (taumax-taumin)
    
    normalpopt =  [freqRescaled, sinphiRescaled, cosphiRescaled, fmodRescaled, ImodRescaled, tauRescaled  ]
...
for dataTarget in tqdm(range(nbSamples)):
    ...
    fitEval, popt,normalpopt, initGuess = doFit(xdata,ydata, initialGuesses, normalizationRange)
...
axs[0].scatter(sortedNoise, sortedMSEfitSignal ,label= "fit" ,c='blue', alpha = 0.8)
axs[0].scatter(sortedNoise, sortedMSEunitySignal, marker="*" ,label= "DNN" ,c='orange', alpha = 0.5)
axs[0].set_yscale('log')
axs[0].legend()
axs[0].set_ylabel("$MSE_{dec}$ [-]", fontsize = fontsizes)
 
axs[1].scatter(sortedNoise, sortedMSEfitLatent  ,label= "fit" ,c='blue', alpha = 0.8)
axs[1].scatter(sortedNoise, sortedMSEunityLatent,  marker="*" ,label= "DNN" ,c='orange', alpha = 0.5)
axs[1].set_yscale('log')
axs[1].legend()
axs[1].set_xlabel("Noise Level [a.u]", fontsize = fontsizes)
axs[1].set_ylabel("$MSE_{reg}$ [-]", fontsize = fontsizes)
...
for selector in selectors:
    ...
    latent_predDNN = unityOutputs[0,512:-1]
    ...
    fig = plt.figure()  
    plt.subplot(311)
    plt.plot(ydata, 'b' ,linewidth=5, label = "Input - noisy AM sine wave")
    plt.plot(true,'orange' ,linewidth=3, label = "Target - noiseless AM-sine wave")
    plt.xticks([])
    # plt.yticks([])
    plt.legend()
    plt.subplot(312)
    plt.plot(true,'orange' ,linewidth=5,  label = "Target - noiseless AM-sine wave")
    plt.plot(predDNN, 'k',linewidth=3, label = "DNN prediction")
    plt.plot(predFIT,'green',linewidth=1, label = "Fit prediction")
    plt.xticks([])
    # plt.yticks([])
    plt.legend()
    plt.subplot(313)
    xbar = np.linspace(0,latentDim-1,latentDim-1)
    plt.bar(xbar,true_latent[:-1])
    plt.bar(xbar,latent_predDNN,width=0.6)
    plt.bar(xbar,latent_predFIT,width=0.4)
    # plt.yticks([])
    plt.legend()
    plt.tight_layout()
    plt.show()
